...
import com.buynow.repository.OrderItemRepository;
...
//		shippAddress.setUser(user);
		...
		for(Map.Entry<Long, List<CartItem>> entry:itemsBySeller.entrySet()){
			...
			int totalMrpOrderPrice = cartItems.stream().mapToInt(CartItem::getMrpPrice).sum();
			
			int totalSellingOrderPrice = cartItems.stream().mapToInt(CartItem::getSellingPrice).sum();
			...
			createdOrder.setTotalMrpPrice(totalMrpOrderPrice);
			createdOrder.setTotalSellingPrice(totalSellingOrderPrice);
			...
			for(CartItem item : cartItems) {
				...
				orderItem.setOrder(savedOrder); // take belongs to this order
				orderItem.setMrpPrice(item.getMrpPrice());// take 
				orderItem.setProduct(item.getProduct());
				orderItem.setQuantity(item.getQuantity());
				orderItem.setSize(item.getSize());
				orderItem.setUserId(item.getUserId());// take 
				orderItem.setSellingPrice(item.getSellingPrice());// take  it u will understd
