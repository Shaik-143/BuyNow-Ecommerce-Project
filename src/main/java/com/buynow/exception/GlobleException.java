...
public class GlobleException {
	...
	public ResponseEntity<ErrorDetails> handleSellerException(SellerException ex, WebRequest req) {
		//Generic interface for a web request.Giving them access to general request metadata
		//or data , not for actual handling of the request.
		ErrorDetails err= new ErrorDetails(ex.getMessage(),req.getDescription(false),LocalDateTime.now());

		return new ResponseEntity<ErrorDetails>(err,HttpStatus.BAD_REQUEST);
		//"error": "Seller not found with id 4",
	    //"details": "uri=/sellers/4", general request data
	    //"timestamp": "2026-03-12T14:06:10.537609"
	}
